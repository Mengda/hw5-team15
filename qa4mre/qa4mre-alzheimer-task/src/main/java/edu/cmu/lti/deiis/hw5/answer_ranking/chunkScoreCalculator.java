...
public class chunkScoreCalculator {
	...
	static ArrayList<ArrayList<String>> groupQuestion = null;

	public static void SetQuestion(String question) {
		chunkScoreCalculator.groupQuestion = CBExtractor(question);
	}

	public static double AnswerSimilarity(String answer) {

		ArrayList<ArrayList<String>> answerChunk = CBExtractor(answer);
		double maxScore = calculateChunkScore(
				chunkScoreCalculator.groupQuestion, answerChunk) / 1.5;
		// System.out.println("max score is " + maxScore);
		return Math.max(maxScore, 0.01d);
	}

	static ReVerbExtractor reverb = new ReVerbExtractor();
	static boolean Flag_CBExtractor_Init = false;
	static OpenNlpSentenceChunker chunker = null;
	static ConfidenceFunction confFunc = null;

	static void CBExtractor_Init() {
		try {
		...
		Flag_CBExtractor_Init = true;
	}

	private static ArrayList<ArrayList<String>> CBExtractor(String str) {
		if (!Flag_CBExtractor_Init) {
			chunkScoreCalculator.CBExtractor_Init();
		}
		String sentStr = str;
		// Looks on the classpath for the default model files.

		ChunkedSentence sent = chunkScoreCalculator.chunker
				.chunkSentence(sentStr);
		...
		for (ChunkedBinaryExtraction extr : chunkScoreCalculator.reverb
				.extract(sent)) {
	...
	public static double calculateChunkScore(
		...
		for (ArrayList<String> q : Question) {
			for (ArrayList<String> a : Answer) {
				...
				currentScore = Math.max(calculateTokenScore(q.get(0), a.get(0))
						+ calculateTokenScore(q.get(2), a.get(2)),
						calculateTokenScore(q.get(0), a.get(2))
								+ calculateTokenScore(q.get(2), a.get(0)))
	...
	public static double calculateTokenScore(String str1, String str2) {
		...
		for (String s1 : tokens1) {
			...
			for (String s2 : tokens2) {
				...
				// System.out.println("word similarity between " + s1 + " and "
				// + s2 + " "
	...
	static boolean Flag_wordSimilarity_Initialize = false;

	static ILexicalDatabase db = new NictWordNet();
	static RelatednessCalculator[] rcs = { new HirstStOnge(db),
			new LeacockChodorow(db), new Lesk(db), new WuPalmer(db),
			new Resnik(db), new JiangConrath(db), new Lin(db), new Path(db) };

	static RelatednessCalculator rc = new Lin(db);

	public static double wordSimilarity(String word1, String word2) {
		if (!chunkScoreCalculator.Flag_wordSimilarity_Initialize) {
			WS4JConfiguration.getInstance().setMFS(true);
			chunkScoreCalculator.Flag_wordSimilarity_Initialize = true;
		}
